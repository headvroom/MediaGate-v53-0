...
class NewVideoClubController extends \Controller\BaseStalkerController {
    ...
    public function video_list_json($local_uses = FALSE) {
        ...
        if (!empty($response['data'])) {
            ...
            while (list($key, $row) = each($response['data'])){
                ...
                $response['data'][$key]['on_storages'] = (int) $this->db->getAvailableFilesByVideoID($row['id'], 'COUNT');
            }
        }
    ...
    public function video_info() {
        ...
        if (is_numeric($this->postData['videoid'])){
            ...
            try{
                ...
                $good_storages = $master->getAllGoodStoragesForMediaFromNet($this->postData['videoid'], 0, true);
            } catch (\Exception $e) {
                $good_storages = array();
    ...
    public function video_advertise_list_json($local_uses = FALSE) {
        ...
        $response["data"] = array_map(function($row) use ($must_watch, $vclub_ads){
        ...
        if (!empty($this->method) && $this->method == 'POST') {
            ...
            if ($form->isValid()) {
                ...
                if (!$is_repeating_name) {
                    ...
                    $error_local = array();
                    $error_local['name'] = ($is_repeating_name ? $this->setLocalization('Name is busy') : '');
