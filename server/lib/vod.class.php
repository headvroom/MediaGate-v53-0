...
class Vod extends AjaxResponse implements \Stalker\Lib\StbApi\Vod
{
    ...
    private function handleTmpLink($url, $file){
        ...
        if ($file['tmp_link_type'] == 'flussonic'){
            ...
            $url .= (strpos($url, '?') ? '&' : '?' ). Itv::getNimbleHttpAuthToken($url, 'NIMBLE_VIDEO_VALID_MINUTES');
            ...
            $url .= (strpos($url, '?') ? '&' : '?' ). Itv::getWowzaSecureToken($url, Config::getSafe('wowza_securetoken_vod_endtime', 0));
        }
    ...
    private function getData()
    {
        ...
        if ((empty($_REQUEST['category']) || $_REQUEST['category'] == '*') && !Config::getSafe('show_adult_movies_in_common_list', true)){
            $not_in_category_ids = Mysql::getInstance()->from('media_category')->where(array('category_alias' => 'adult', 'censored' => 1), 'OR ')->get()->all('id');
        }
